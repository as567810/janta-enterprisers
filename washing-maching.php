<!DOCTYPE html>
<html lang="en">
<?php include('include/head.php')?>

<body>
  <?php include('include/header.php')?>

  <section class="banner-sec">
    <img class="desk" src="images/washing-machine-repair.jpg" alt="" />
    <img class="mob" src="images/washing-machine-repair.jpg" alt="" />
    <div class="banner-content">
      <!-- <h2>PROVIDE QUALITY<br />REPAIR SERVICE</h2> -->
      <h2>Washing Machine Repair Services in Greater Noida</h2>
    </div>
  </section>

  <section class="define">
    <div class="container">
      <div class="main_def">
        <div class="main-left">
          <div class="repair">
            <h2>Providing best washing machine repairing services in Noida</h2>
            <p>We are proud to be the best <strong>washing machine repair in Greater Noida</strong>. We have a team full of highly trained technicians who can repair all brands of dryers and washers. The technicians are highly skilled and have the experience to resolve the most complex problems. The following services are offered by our company:</p>
            <ul class="first-rep">
              <li>We offer fast, efficient and competitive service for washing machines. We can accommodate any type of fixture.</li>
              <li>We are a leading service provider in the industry, offering the best twin tub washer spin dryer repair. Our team includes professionals and experts who can repair all types of washers and dryers.</li>
            </ul>
          </div>

          <div class="repair">
            <h3>Washing Machine Repair Service Center in Greater Noida</h3>
            <p>A washing machine is an excellent addition to your home. <strong>Washing Machine repair</strong> services is available to fix any problems. The <strong>Janta Enterprises</strong> will repair your washing machine to ensure that it performs well. This is due to the highly-skilled, well-trained professionals. <br /><br /> We have many years of washing machine repair experience and can fix any faults in a matter of minutes. We will do our best to solve your problems at home.</p>
          </div>
          <!-- <div class="repair">
            <h3>Washing Machine Problem:</h3>
            <ul>
              <li>Washing Machine starting problem</li>
              <li>Washing Machine starting problem</li>
              <li>Washing Machine starting problem</li>
              <li>Washing Machine starting problem</li>
              <li>Washing Machine starting problem</li>
              <li>Washing Machine starting problem</li>
              <li>Washing Machine starting problem</li>
              <li>Washing Machine starting problem</li>
              <li>Washing Machine starting problem</li>
              <li>Washing Machine starting problem</li>
            </ul>
          </div> -->
        </div>
        <div class="main-right">
          <div class="repair">
            <h2>All Brands</h2>
            <ul>
              <li>Godrej</li>
              <li>Whirlpool</li>
              <li>Samsung</li>
              <li>LG</li>
              <li>IFB</li>
              <li>Panasonic</li>
              <li>Videocon</li>
              <li>Voltas</li>
              <!-- <li>Voltas Washing Machine repairing</li>
              <li>Voltas Washing Machine repairing</li>
              <li>Voltas Washing Machine repairing</li>
              <li>Voltas Washing Machine repairing</li>
              <li>Voltas Washing Machine repairing</li>
              <li>Voltas Washing Machine repairing</li> -->
            </ul>
          </div>
          <div class="repair">
            <h2>Why Choose us</h2>
            <ul>
              <li>No extra charge​</li>
              <li>24/7 emergency service​</li>
              <li>Expert Technician's​</li>
              <li>Customer Satisfaction​</li>
              <li>On Time Service​</li>

            </ul>
          </div>
        </div>
      </div>



    </div>
  </section>
  <section class="reviews">
    <div class="container">
      <div class="main-d">
        <div class="comm-d">
          <figure>
            <img src="images/services/wash/1.jpg" alt="">
          </figure>
          <div class="low">
            <h2>Front-Loading Washing Machines</h2>
           
            <!-- <p>Our highly-trained and experienced technicians will quickly find and fix any fault to your machine and complete the repair in a matter of minutes.</p> -->
          </div>
        </div>
        <div class="comm-d">
          <figure>
            <img src="images/services/wash/2.jpg" alt="">
          </figure>
          <div class="low">
            <h2>Washer And Dryer Combo</h2>
            <!-- <div class="ratings"> <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star"></span>
              <span class="fa fa-star"></span>
            </div>
            <p>Our latest technology and experienced technicians allow us to repair any type of problem on the machine.</p> -->
          </div>
        </div>
        <div class="comm-d">
          <figure>
            <img src="images/services/wash/3.jpg" alt="">
          </figure>
          <div class="low">
            <h2> Integrated</h2>
            <!-- <div class="ratings"> <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star"></span>
              <span class="fa fa-star"></span>
            </div>
            <p>Each service and spare part comes with warranty.</p> -->
          </div>
        </div>
        <div class="comm-d">
          <figure>
            <img src="images/services/wash/4.jpg" alt="">
          </figure>
          <div class="low">
            <h2>Stackable</h2>
            <!-- <div class="ratings"> <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star"></span>
              <span class="fa fa-star"></span>
            </div>
            <p>Our team includes highly-skilled technicians who can provide fast, reliable service.</p> -->
          </div>
        </div>
        <div class="comm-d">
          <figure>
            <img src="images/services/wash/5.jpg" alt="">
          </figure>
          <div class="low">
            <h2>Portable Or Compact</h2>
            <!-- <div class="ratings"> <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star"></span>
              <span class="fa fa-star"></span>
            </div>
            <p>Accessible 24/7 at very affordable prices.</p> -->
          </div>
        </div>
        <div class="comm-d">
          <figure>
            <img src="images/services/wash/6.jpg" alt="">
          </figure>
          <div class="low">
            <h2>Top-Loading</h2>
            <!-- <div class="ratings"> <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star"></span>
              <span class="fa fa-star"></span>
            </div>
            <p>Our latest technology and experienced technicians allow us to repair any type of problem on the machine.</p> -->
          </div>
        </div>
        <div class="comm-d">
          <figure>
            <img src="images/services/wash/7.jpg" alt="">
          </figure>
          <div class="low">
            <h2>Semi-Automatic</h2>
            <!-- <div class="ratings"> <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star"></span>
              <span class="fa fa-star"></span>
            </div>
            <p>Our latest technology and experienced technicians allow us to repair any type of problem on the machine.</p> -->
          </div>
        </div>
        <div class="comm-d">
          <figure>
            <img src="images/services/wash/8.jpg" alt="">
          </figure>
          <div class="low">
            <h2> Fully-Automatic</h2>
            <!-- <div class="ratings"> <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star checked"></span>
              <span class="fa fa-star"></span>
              <span class="fa fa-star"></span>
            </div>
            <p>Our latest technology and experienced technicians allow us to repair any type of problem on the machine.</p> -->
          </div>
        </div>
    
      </div>
    </div>
  </section>

  <?php include('include/footer.php')?>
</body>

</html>